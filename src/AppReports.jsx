...
const text = {
  fr: {
    ...
    subtitle:
      "Gère tes commandes, acomptes, livraisons, clientes, charges et rapports depuis ton téléphone.",
    dashboard: "Accueil",
    add: "Ajouter",
    orders: "Commandes",
    payments: "Paiements",
    ...
    contact: "Contact",
    instagram: "Instagram",
    contactHelp: "Écris ton message puis contacte ShopFlow DZ sur Instagram.",
    today: "Aujourd'hui",
    week: "Cette semaine",
    month: "Ce mois",
    all: "Tout",
    ...
    reportsIntro:
      "Analyse rapidement ce qui se vend, ce qui reste à encaisser et quelles charges pèsent sur ton bénéfice net.",
  },
  ar: {
    login: "تسجيل الدخول",
    register: "إنشاء حساب",
    ...
    subtitle:
      "سيّري الطلبات، العربون، التوصيل، الزبائن، المصاريف والتقارير من الهاتف.",
    dashboard: "الرئيسية",
    add: "إضافة",
    orders: "الطلبات",
    payments: "الدفع",
    ...
    contact: "تواصل",
    instagram: "إنستغرام",
    contactHelp: "اكتبي رسالتك ثم تواصلي مع ShopFlow DZ عبر إنستغرام.",
    today: "اليوم",
    week: "هذا الأسبوع",
    month: "هذا الشهر",
    all: "الكل",
    ...
    reportsIntro:
      "حللي بسرعة ما يُباع، ما تبقى للتحصيل، والمصاريف التي تؤثر على الربح الصافي.",
  },
};
...
export default function AppReports() {
  ...
  const reports = useMemo(
    () => buildReports(orders, expenses, period),
  ...
  async function updatePayment(order, deposit) {
    ...
    if (value < 0 || value > Number(order.sellingPrice || 0)) {
  ...
  const appClass = dark
    ? "min-h-screen bg-[#07142B] text-white pb-28"
    : "min-h-screen bg-[#F5F8FC] text-slate-950 pb-28";
        ...
        <div className="mx-auto max-w-md px-5 py-8">
          ...
          <section className={card(dark, "mt-6 text-center")}>
            ...
            <div className="mt-5 grid grid-cols-2 gap-2 rounded-2xl bg-slate-100/70 p-1">
              <button onClick={() => setAuthMode("login")} className={tab(authMode === "login", dark)}>
                {t.login}
              </button>
              <button onClick={() => setAuthMode("register")} className={tab(authMode === "register", dark)}>
                {t.register}
              </button>
            </div>
            ...
            <form onSubmit={authMode === "login" ? login : register} className="mt-5 space-y-3 text-start">
              {authMode === "register" && (
                <Field label={t.shopName} value={shopName} onChange={(e) => setShopName(e.target.value)} dark={dark} />
              )}
              ...
              <button disabled={loading} className="primary-btn w-full">
                {loading ? "..." : authMode === "login" ? t.connect : t.create}
              </button>
            </form>
    ...
    <main dir={dir} className={appClass}>
      <header className={dark ? "sticky top-0 z-20 border-b border-white/10 bg-[#07142B]/95 backdrop-blur" : "sticky top-0 z-20 border-b border-slate-200 bg-white/95 backdrop-blur"}>
        <div className="mx-auto flex max-w-2xl items-center gap-3 px-4 py-3">
          <img src="/shopflow-logo.svg" alt="ShopFlow DZ" className="w-32" />
          <p className={muted(dark, "min-w-0 flex-1 truncate text-xs")}>{user.email}</p>
          <Controls lang={lang} setLang={setLang} theme={theme} setTheme={setTheme} small />
          <button onClick={logout} className="rounded-2xl bg-red-50 px-3 py-2 text-xs font-bold text-red-600">
            {t.logout}
          </button>
        </div>
      </header>
      ...
      <div className="mx-auto max-w-2xl px-4 py-5">
        ...
        {page === "add" && <AddOrder form={orderForm} setForm={setOrderForm} submit={addOrder} loading={loading} t={t} dark={dark} />}
        {page === "orders" && <Orders orders={orders} updateStatus={updateStatus} deleteOrder={deleteOrder} t={t} dark={dark} />}
        {page === "payments" && <Payments orders={orders} updatePayment={updatePayment} t={t} dark={dark} />}
        {page === "delivery" && <Delivery orders={orders} updateDelivery={updateDelivery} t={t} dark={dark} />}
        {page === "expenses" && <Expenses expenses={expenses} form={expenseForm} setForm={setExpenseForm} submit={addExpense} deleteExpense={deleteExpense} loading={loading} t={t} dark={dark} />}
        ...
        {page === "more" && <More t={t} dark={dark} />}
      </div>

      <a href="https://www.instagram.com/shopflowdz/" target="_blank" rel="noreferrer" className="fixed bottom-24 right-4 z-30 rounded-full bg-gradient-to-r from-[#00E5FF] to-[#00C4B4] px-4 py-3 text-sm font-black text-[#07142B] shadow-xl">
        Instagram
      </a>
  ...
  return (
    <section className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <Quick onClick={() => setPage("expenses")} label={t.addExpense} />
        <Quick onClick={() => setPage("reports")} label={t.reports} />
        <Quick onClick={() => setPage("delivery")} label={t.delivery} />
      </div>
    </section>
  );
}

function AddOrder({ form, setForm, submit, loading, t, dark }) {
  ...
  return (
    <section className={card(dark)}>
      ...
      <form onSubmit={submit} className="mt-4 space-y-3">
        ...
        <Select label={t.status} value={form.status} onChange={(e) => set("status", e.target.value)} options={STATUSES} dark={dark} />
        <div className="grid grid-cols-2 gap-3">
...
function Orders({ orders, updateStatus, deleteOrder, t, dark }) {
  const [q, setQ] = useState("");
  const shown = orders.filter((order) =>
    `${order.customerName} ${order.customerPhone} ${order.productName}`
      .toLowerCase()
  ...
  return (
    <section className="space-y-3">
      ...
      {shown.map((order) => (
        <article key={order.id} className={card(dark)}>
          <div className="flex justify-between gap-3">
            <div>
              <h3 className="font-black">{order.customerName}</h3>
              <p className={muted(dark, "text-sm")}>{order.productName}</p>
            </div>
            <span className="h-fit rounded-full bg-cyan-50 px-3 py-1 text-xs font-black text-cyan-700">
          ...
          <div className="mt-3 flex gap-2">
            <a href={wa(order.customerPhone, `Bonjour ${order.customerName}, concernant votre commande ${order.productName}.`)} target="_blank" rel="noreferrer" className="flex-1 rounded-2xl bg-green-50 px-4 py-3 text-center text-sm font-black text-green-700">
              WhatsApp
            </a>
            <button onClick={() => deleteOrder(order)} className="rounded-2xl bg-red-50 px-4 py-3 text-sm font-black text-red-600">
              {t.del}
            </button>
          </div>
        </article>
...
function Payments({ orders, updatePayment, t, dark }) {
  return (
    <section className="space-y-3">
      ...
      {orders.filter((o) => o.status !== "Annulée" && o.status !== "Retour").map((order) => (
        <Payment key={order.id} order={order} updatePayment={updatePayment} t={t} dark={dark} />
      ))}
    </section>
  );
}
...
function Delivery({ orders, updateDelivery, t, dark }) {
  return (
    <section className="space-y-3">
      <Title title={t.delivery} dark={dark} />
      {orders.filter((o) => o.status !== "Nouvelle" && o.status !== "Annulée").map((order) => (
        <DeliveryItem key={order.id} order={order} updateDelivery={updateDelivery} t={t} dark={dark} />
      ))}
    </section>
  );
}

function DeliveryItem({ order, updateDelivery, t, dark }) {
  ...
  return (
    <article className={card(dark)}>
      ...
      <div className="mt-4 space-y-3">
        <Select label={t.status} value={form.status} onChange={(e) => setForm({ ...form, status: e.target.value })} options={DELIVERY_STATUSES} dark={dark} />
...
function Expenses({ expenses, form, setForm, submit, deleteExpense, loading, t, dark }) {
  const set = (key, value) => setForm({ ...form, [key]: value });
    ...
    <section className="space-y-4">
      <div className={card(dark)}>
        ...
        <form onSubmit={submit} className="mt-4 space-y-3">
          ...
          <Select label={t.category} value={form.category} onChange={(e) => set("category", e.target.value)} options={EXPENSE_CATEGORIES} dark={dark} />
          <Field label={t.note} value={form.note} onChange={(e) => set("note", e.target.value)} dark={dark} />
          <button disabled={loading} className="primary-btn w-full">{loading ? "..." : t.save}</button>
        </form>
      </div>
      ...
      {expenses.map((expense) => (
        <article key={expense.id} className={card(dark)}>
          <div className="flex justify-between gap-3">
            <div>
              <h3 className="font-black">{expense.label}</h3>
              <p className={muted(dark, "text-sm")}>{expense.category}</p>
            </div>
            <strong>{money(expense.amount)}</strong>
          </div>
          {expense.note && <p className={muted(dark, "mt-2 text-sm")}>{expense.note}</p>}
          <button onClick={() => deleteExpense(expense)} className="mt-3 rounded-2xl bg-red-50 px-4 py-3 text-sm font-black text-red-600">
            {t.del}
          </button>
        </article>
      ))}
    ...
    <section className="space-y-4">
      <div className={card(dark)}>
        ...
        <div className="mt-4 grid grid-cols-4 gap-2">
          {PERIODS.map((item) => (
            <button key={item} onClick={() => setPeriod(item)} className={period === item ? "rounded-2xl bg-[#07142B] px-2 py-3 text-xs font-black text-white" : dark ? "rounded-2xl bg-white/5 px-2 py-3 text-xs font-bold text-slate-300" : "rounded-2xl bg-slate-100 px-2 py-3 text-xs font-bold text-slate-600"}>
              {t[item]}
            </button>
          ))}
        </div>
    ...
    <section className="space-y-3">
      ...
      {clients.map((client) => (
        <article key={client.id} className={card(dark)}>
          ...
          {client.phone && (
            <a href={wa(client.phone, `Bonjour ${client.name}, c'est ShopFlow DZ.`)} target="_blank" rel="noreferrer" className="mt-3 block rounded-2xl bg-green-50 px-4 py-3 text-center text-sm font-black text-green-700">
              WhatsApp
            </a>
          )}
        </article>
...
function More({ t, dark }) {
  const [contact, setContact] = useState({ name: "", phone: "", message: "" });
  const msg = `Bonjour ShopFlow DZ, je suis ${contact.name || "..."} (${contact.phone || "..."}). ${contact.message || "Je veux vous contacter."}`;
  ...
  return (
    <section className="space-y-4">
      ...
      <div className={card(dark)}>
        ...
        <div className="mt-4 space-y-3">
          <Field label="Nom" value={contact.name} onChange={(e) => setContact({ ...contact, name: e.target.value })} dark={dark} />
          <Field label="Téléphone" value={contact.phone} onChange={(e) => setContact({ ...contact, phone: e.target.value })} dark={dark} />
          <Field label="Message" value={contact.message} onChange={(e) => setContact({ ...contact, message: e.target.value })} dark={dark} />
          <a href="https://www.instagram.com/shopflowdz/" target="_blank" rel="noreferrer" onClick={() => navigator.clipboard?.writeText(msg)} className="primary-btn block text-center">
            {t.instagram}
          </a>
          <p className={muted(dark, "text-xs")}>Le message est copié automatiquement si le navigateur l'autorise.</p>
        </div>
      </div>
    </section>
  );
...
function Nav({ page, setPage, t, dark }) {
  ...
  return (
    <nav className={dark ? "fixed bottom-0 left-0 right-0 z-20 border-t border-white/10 bg-[#07142B]/95 backdrop-blur" : "fixed bottom-0 left-0 right-0 z-20 border-t border-slate-200 bg-white/95 backdrop-blur"}>
      <div className="mx-auto grid max-w-2xl grid-cols-8 gap-1 px-2 py-2">
        {items.map(([key, label]) => (
          <button key={key} onClick={() => setPage(key)} className={page === key ? "rounded-2xl bg-gradient-to-r from-[#00E5FF] to-[#00C4B4] px-1 py-3 text-[9px] font-black text-[#07142B]" : `rounded-2xl px-1 py-3 text-[9px] font-bold ${dark ? "text-slate-300" : "text-slate-500"}`}>
            {label}
          </button>
        ))}
      </div>
...
function Controls({ lang, setLang, theme, setTheme, small }) {
  return (
    <div className={`flex ${small ? "gap-1" : "justify-end gap-2"}`}>
      <button onClick={() => setLang(lang === "fr" ? "ar" : "fr")} className="rounded-full border border-cyan-300/40 bg-white/10 px-3 py-2 text-xs font-black">
        {lang === "fr" ? "AR" : "FR"}
      </button>
      <button onClick={() => setTheme(theme === "light" ? "night" : "light")} className="rounded-full border border-cyan-300/40 bg-white/10 px-3 py-2 text-xs font-black">
        {theme === "light" ? "🌙" : "☀️"}
      </button>
    </div>
  );
...
function Select({ label, options, dark, ...props }) {
  return (
    <label className="block">
      <span className={muted(dark, "mb-1 block text-xs font-bold")}>{label}</span>
...
function Stat({ label, value, dark, hot }) {
  return (
    <div className={hot ? "rounded-3xl bg-gradient-to-r from-[#00E5FF] to-[#00C4B4] p-4 text-[#07142B]" : dark ? "rounded-3xl bg-white/5 p-4" : "rounded-3xl bg-slate-50 p-4"}>
      <p className={`text-xs font-bold ${hot ? "text-[#07142B]/70" : dark ? "text-slate-300" : "text-slate-500"}`}>{label}</p>
...
function Mini({ label, value, dark }) {
  return (
    <div className={dark ? "rounded-2xl bg-white/5 p-3" : "rounded-2xl bg-slate-50 p-3"}>
      <p className={muted(dark, "text-[11px] font-bold")}>{label}</p>
...
function LinkBox({ href, title, dark }) {
  return (
    <a href={href} target={href.startsWith("http") ? "_blank" : undefined} rel={href.startsWith("http") ? "noreferrer" : undefined} className={dark ? "rounded-3xl bg-white/5 p-4 font-black text-white" : "rounded-3xl bg-white p-4 font-black text-slate-900 shadow-sm"}>
      {title}
    </a>
  );
}
...
function aggregateRows(items, key, fallback, amountKey, withCount) {
  ...
  return Array.from(map.values())
    ...
    .map((row) => ({
      label: row.label,
      meta: withCount ? `${row.count} ${row.count > 1 ? "commandes" : "commande"}` : `${row.count} ligne${row.count > 1 ? "s" : ""}`,
